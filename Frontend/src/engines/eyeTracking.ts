...
import type { AttentionStateName } from "./attentionModel";
...
const STATE_PROFILES: Record<AttentionStateName, Record<MetricName, [number, number]>> = {
...
function ear(lm: Landmark[], indices: number[], w: number, h: number): number {
  const c = indices.map((i) => [lm[i].x * w, lm[i].y * h]);
  return (dist(c[1], c[5]) + dist(c[2], c[4])) / (2 * dist(c[0], c[3]) + 1e-6);
}

function irisDiam(lm: Landmark[], indices: number[], w: number, h: number): number {
...
function irisCenter(lm: Landmark[], indices: number[], w: number, h: number): [number, number] {
...
type ResultCallback = (result: EyeResult) => void;

export class EyeTracker {
  readonly W: number;
  private pupilBuf: number[] = [];
  private gazeBuf: Array<[number, number]> = [];
  private fixBuf: number[] = [];
  private saccBuf: number[] = [];
  private blinkWin: number[] = [];
  private baseline: number | null = null;
  private lastPos: [number, number] | null = null;
  private fixStart: number | null = null;
  state: AttentionStateName = "Focused";
  conf = 0.5;
  private frame = 0;
  private blinks = 0;
  private consec = 0;
  private EAR_THRESHOLD = 0.21;
  ...
  private _loop() {
    ...
    if (result?.faceLandmarks?.length) {
      const metrics = this._processLandmarks(
        result.faceLandmarks[0],
        this.video.videoWidth,
        this.video.videoHeight
      );
      if (this._onResult) {
        this._onResult({ state: this.state, confidence: this.conf, metrics });
      }
    }
  ...
  private _processLandmarks(landmarks: Landmark[], w: number, h: number): EyeMetrics {
    this.frame++;
    const lm = landmarks;
    ...
    if (this.lastPos) {
      ...
      if (mv > 30) {
        ...
        if (this.fixStart) {
          this.fixBuf.push(now - this.fixStart);
          this.fixStart = null;
        }
    ...
    const metrics: EyeMetrics = {
      ...
      pupil_dilation: this.baseline
        ? ((this.pupilBuf.slice(-10).reduce((s, v) => s + v, 0) /
            Math.min(10, this.pupilBuf.length) -
            this.baseline) /
            this.baseline) *
          100
        : 0,
      fixation_duration:
        this.fixBuf.length > 0
          ? this.fixBuf.slice().sort((a, b) => a - b)[Math.floor(this.fixBuf.length / 2)]
          : 300,
      saccade_rate: this.saccBuf.length / 2,
      gaze_stability:
        this.gazeBuf.length >= 10
          ? Math.max(
              0,
              1 -
                this.gazeBuf.slice(-10).reduce((s, g) => s + dist(g, [w / 2, h / 2]), 0) /
                  10 /
                  200
            )
          : 1.0,
  ...
  private _classify(m: EyeMetrics): [AttentionStateName, number] {
    const scores: Partial<Record<AttentionStateName, number>> = {};
    for (const [state, profile] of Object.entries(STATE_PROFILES) as Array<
      [AttentionStateName, Record<MetricName, [number, number]>]
    >) {
      let tot = 0;
      let n = 0;
      for (const [key, [lo, hi]] of Object.entries(profile) as Array<[MetricName, [number, number]]>) {
        if (!(key in m)) continue;
        const v = m[key];
        tot +=
          lo <= v && v <= hi
            ? 1
            : Math.max(0, 1 - Math.min(Math.abs(v - lo), Math.abs(v - hi)) / ((hi - lo || 1) * 2));
        n++;
      }
      scores[state] = n > 0 ? tot / n : 0;
    }
    const best = (Object.entries(scores) as Array<[AttentionStateName, number]>).sort(
      (a, b) => b[1] - a[1]
    )[0];
  ...
  getState(): EyeSnapshotLite {
    return { state: this.state, confidence: this.conf };
  }
